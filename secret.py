...
lives = int(input("How many times do you want to try? "))
...
def update(guess, secret, clue, unknown):
    index = 0
...
while lives> 0:
    ...
    guessed = input('guess a letter or the whole word: ')

    if guessed == secret:
        correct = True
        break
    if guessed in secret:
        unknown = update(guessed, secret, clue, unknown)
    else:
        print('INCORRECT')
        lives = lives - 1
    if unknown == 0:
        ...
        break
if correct == True:
    print('You won! The secret was ' + secret)
else:
